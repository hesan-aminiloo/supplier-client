...
import { useSystemUsersForm } from '../hooks/AddSystemUsers.tools';
import { SystemUserAddDrawerProps } from '../SystemUsers.types';
import { FORM_ELEMENTS } from './Form/Form.constants';
import { EnumRoles, FormElementProps } from './Form/Form.types';

const AddSystemUserDrawer: React.FC<SystemUserAddDrawerProps> = ({ isOpen, onClose, branches, getSystemUsersList }) => {
  const [filePreview, setFilePreview] = useState<any>('');
  const [file, setFile] = useState<any>();
  const [logoSize, setLogoSize] = useState<number>(0);
  const [role, setRole] = useState<string>('');
  const { methods, onSubmit, isLoading } = useSystemUsersForm({ onClose, getSystemUsersList, setFilePreview });
  ...
  return (
    ...
    >
      <DrawerHeader
        title={t('settings.system_users.add_system_user')}
        onClose={onClose}
      />
      ...
      <div className="pt-6 px-6 bg-neutral-100 h-full">
        <FormProvider {...methods}>
          ...
          >
            ...
            <DrawerFooter
              title={t('settings.system_users.add_user')}
              onSubmit={methods.handleSubmit(onSubmit)}
            />
          </form>
        </FormProvider>
...
export default AddSystemUserDrawer;
